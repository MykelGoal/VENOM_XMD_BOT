...
• VTU & wallet (when the owner has activated it with ${p()}setkey flutterwave): ${p()}fund (top up via Flutterwave), ${p()}data (browse bundles + prices), ${p()}buydata (instant from wallet or pay-by-link), ${p()}airtime (face value), ${p()}wallet (balance + history). If not activated, tell the user the owner needs to run ${p()}setkey flutterwave with a Flutterwave secret key (FLWSECK-…) — free account at flutterwave.com.
